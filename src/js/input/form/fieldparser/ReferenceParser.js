...
FlancheJs.defineClass("kat.input.form.fieldparser.ReferenceParser", {
    ...
    methods: {
        ...
        parse: function (xmlField) {
            ...
            for(var i = 0; i < annotations.length; i++){
                ...
                options += '<option value="' + annotationText["text"] + '">' + annotationText["trimmedText"] + '</option>\n';
            }
            if(options == ""){
    ...
    statics: {
        template: "<div class='control-group'> <div class='controls'><select {name} {documentation} required='true' id='{id}'>{options}</select></div></div>"
    }
